...
import 'package:tfg_v3/src/features/authentication/screens/landing/landing_screen.dart';
import 'package:tfg_v3/src/features/authentication/screens/mail_verification/mail_verification.dart';
import 'package:tfg_v3/src/features/menu/screens/menu_screen.dart';
import 'package:tfg_v3/src/repository/authentication_repository/exceptionds.dart';
    ...
    try {
      await _auth.sendPasswordResetEmail(email: email).then(
        (value) {
          getSnackBar(
            "Success",
            "Password reset email link has been sent, please change password and login again :)",
            false,
          );
          logout();
        },
      ...
      await user!.reauthenticateWithCredential(credential).then(
        (value) {
          value.user!.delete().then(
            (res) {
              getSnackBar("Succes", "User account has been deleted successfully", false);
            },
          );
        },
      );
