...
import 'package:tfg_v3/src/features/generators/screens/chat_generator/chat_widget.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> sendMessage({
    ...
    if (_isTyping) {
      ScaffoldMessenger.of(context).showSnackBar(snackBarError(tMultipleMeassages));
      return;
    }
    if (_textEditingController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(snackBarError(tTypeMeassage));
      return;
    }
    try {
      ...
      if (error.toString().contains("HttpException")) {
        errorText = tModelOverloaded;
      }

      if (error.toString().contains("max_length")) {
        errorText = tMaximumTokens;
